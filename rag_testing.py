...
class DenseRetriever(Retriever):
    def __init__(self, retriever_trained=True, model_modified=True):
        ...
        if retriever_trained:
            ...
            self.model = torch.load("/shared/xiaocong/graph_rag/bge-large-en-v1.5_math23k_10epoch_v6_masked.pt", map_location="cpu")
            # print(self.model)
        ...
        self.corpus_embeddings, self.corpus = self._encode_corpus("openai/gsm8k")
        # if self.retriever_trained:
        #     torch.save(self.corpus_embeddings, "metamathqa_corpus_embeddings_trained_v1.pt")
        #     with open("metamathqa_corpus_trained_v1.json", "w") as f:
        #         f.write(json.dumps(self.corpus, ensure_ascii=False))
        # else:
        #     torch.save(self.corpus_embeddings, "metamathqa_corpus_embeddings_untrained_v1.pt")
        #     with open("metamathqa_corpus_untrained_v1.json", "w") as f:
        #         f.write(json.dumps(self.corpus, ensure_ascii=False))
        # self.corpus_embeddings = torch.load("metamathqa_corpus_embeddings_trained_v1.pt").to("cuda:0")
        # with open("metamathqa_corpus_trained_v1.json", "r") as f:
        ...
        # torch.save(self.corpus_embeddings, "gaokao_math_corpus_embeddings_untrained_v1_Chinese.pt")
        # # # save corpus
        # with open("ape210k_corpus_trained_v1_Chinese.json", "w") as f:
        #     f.write(json.dumps(self.corpus, ensure_ascii=False))
        # if not self.retriever_trained:
        #     self.corpus_embeddings = torch.load("/shared/xiaocong/graph_rag/math23k_corpus_embeddings_untrained_Chinese.pt", map_location="cpu").to("cuda:0")
        #     self.corpus = load_dataset("Gxg/Math23K", split="train")
        # else:
        #     self.corpus_embeddings = torch.load("/shared/xiaocong/graph_rag/math23k_corpus_embeddings_trained_v6.2_Chinese.pt").to("cuda:2")
        #     self.corpus = load_dataset("Gxg/Math23K", split="train")
    ...
    def _encode_corpus(self, corpus):
        ...
        if corpus == "gaokao_math":
            # corpus = load_dataset("json",data_files="/shared/xiaocong/ape210k/data/train.ape.jsonl", split="train")
            corpusA = load_dataset("json",data_files="/shared/xiaocong/GAOKAO-Bench/Data/Objective_Questions/2010-2022_Math_I_MCQs.json", split="train")["example"]
            corpusB = load_dataset("json",data_files="/shared/xiaocong/GAOKAO-Bench/Data/Objective_Questions/2010-2022_Math_II_MCQs.json", split="train")["example"]
            # combine two datasets
            corpus = corpusA + corpusB
        else:
            corpus = load_dataset(corpus, "main", split="train")
        all_embeddings = []
        all_data = {"question": [], "answer": []}
        dataloader = torch.utils.data.DataLoader(corpus, batch_size=16, shuffle=False)
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                all_data["answer"].extend(batch["answer"])
...
if __name__ == "__main__":
    # retriever = DenseRetriever(retriever_trained=False, model_modified=False)
    # generator = AutoModelForCausalLM.from_pretrained('meta-llama/Llama-3.2-1B-Instruct', torch_dtype=torch.bfloat16).to("cuda:3")
    pipeline = pipeline("text-generation", model="meta-llama/Llama-3.2-1B-Instruct",torch_dtype=torch.bfloat16,device="cuda:0")
    
    # tokenizer = AutoTokenizer.from_pretrained('meta-llama/Llama-3.2-1B-Instruct')
    # tokenizer.pad_token = tokenizer.eos_token
    # train_dataset = load_dataset("Gxg/Math23K", split="train")
    # train_dataset = load_dataset("json",data_files="/shared/xiaocong/TAL-SCQ5K/ch_single_choice_constructed_5K/ch_single_choice_train_3K.jsonl", split="train")
    ...
    # with open("gsm8k_val_results_trained_8_shots_qwen2.5.jsonl" if retriever.retriever_trained else "gsm8k_val_results_untrained_8_shots_qwen2.5.jsonl", "w") as f:
    
    for batch in tqdm(val_dataloader):
        with open("gsm8k_test_results_randomly_8_shots_llama3.2_em_maj_20_samples.jsonl", "a+") as f:
            prompts = []
            ...
            for i in range(len(batch["question"])):
                ...
                for _ in range(8):
                    ...
                    prompt.append({"role": "assistant", "content": train_dataset["answer"][random_index]})
                prompt.append({"role": "user", "content": batch["question"][i]})
                prompts.append(prompt)
            outputs = pipeline(prompts, max_new_tokens=512, num_return_sequences=10)
            for i, output in enumerate(outputs):
                line = {"answer": batch["answer"][i], "generated": [output[j]["generated_text"][-1]["content"] for j in range(len(output))]}
