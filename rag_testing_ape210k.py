...
class DenseRetriever(Retriever):
    def __init__(self, retriever_trained=True, model_modified=True, trained_retriever_path=None, corpus_dir=None, device=None):
        ...
        if retriever_trained:
            ...
            self.model = AutoModel.from_pretrained('BAAI/bge-large-zh-v1.5',torch_dtype=torch.bfloat16)
    ...
    def _encode_corpus(self, corpus_path):
        ...
        corpus = load_dataset("json",data_files=corpus_path + 'train.ape.jsonl', split="train")
        all_embeddings = []
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                input_texts = batch["original_text"]
                ...
                all_data["question"].extend(batch["original_text"])
...
if __name__ == "__main__":
    ...
    parse.add_argument("--corpus_dir", type=str, default="/shared/xiaocong/ape210k/data/")
    parse.add_argument("--output_path", type=str, default="ape210k_untrained_8_shots_Qwen3B_v4.jsonl")
    parse.add_argument("--trained_retriever_path", type=str, default="/shared/xiaocong/graph_rag/bge-large-zh-v1.5_math23k_25%_data_masked.pt")
    parse.add_argument("--generator", type=str, default="Qwen/Qwen2.5-3B-Instruct")
    parse.add_argument("--retriever_device", type=str, default="cuda:5")
    parse.add_argument("--generator_device", type=str, default="cuda:6")
    ...
    train_dataset = load_dataset("json",data_files=f"{args.corpus_dir}/train.ape.jsonl", split="train")
    val_dataset = load_dataset("json",data_files=f"{args.corpus_dir}/test.ape.jsonl", split="train")
    ...
    for batch in tqdm(val_dataloader):
        with open(args.output_path, "a+") as f:
            prompts = []
            if not args.random:
                retrieved_indices = retriever.retrieve(batch["original_text"], topk=args.topk)
            for i in range(len(batch["original_text"])):
                prompt = [{"role":"system", "content":"模仿给定的格式回答问题。"}]
                if not args.random:
                ...
                prompt.append({"role": "user", "content": batch["original_text"][i]})
