...
class DenseRetriever(Retriever):
    ...
    def _encode_corpus(self, corpus_path):
        ...
        corpus = load_dataset(corpus_path, "raw", split="train")
        all_embeddings = []
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                all_data["question"].extend([batch["question"][i] + "\nOptions" + batch["options"][i] for i in range(len(batch["question"]))])
...
if __name__ == "__main__":
    ...
    parse.add_argument("--corpus_dir", type=str, default="deepmind/aqua_rat")
    parse.add_argument("--output_path", type=str, default="aqua_rat_trained_8_shots_llama3B.jsonl")
    ...
    train_dataset = load_dataset(args.corpus_dir, "raw", split="train")
    val_dataset = load_dataset(args.corpus_dir, "raw", split="test")
