    ...
    def _encode_corpus(self, corpus_path):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                all_data["question"].extend([batch["Problem"][i] + "\nOptions" + batch["options"][i] for i in range(len(batch["Problem"]))])
                all_data["answer"].extend([batch["Rationale"][i] + "#### Answer: " + batch["correct"][i] for i in range(len(batch["Rationale"]))])
...
if __name__ == "__main__":
    ...
    parse.add_argument("--corpus_dir", type=str, default="allenai/math_qa")
    parse.add_argument("--output_path", type=str, default="math_qa_trained_8_shots_llama3B.jsonl")
    ...
    for batch in tqdm(val_dataloader):
        with open(args.output_path, "a+") as f:
            prompts = []
            if not args.random:
                retrieved_indices = retriever.retrieve(batch["Problem"], topk=args.topk)
